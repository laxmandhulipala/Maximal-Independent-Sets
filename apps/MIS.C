...
int runLubyIteration(graph<vertex>& G, bool* inMis, bool* active, bool* selected, int* priorities) {
  const intE n = G.n;

  {parallel_for(long i=0;i<n;i++) selected[i] = true;}
...
void Compute(graph<vertex>& G, commandLine P) {
  ...
  while (numActive > 0) {
    ...
    numActive = runLubyIteration(G, inMis, active, selected, priorities);
    round++;
  }
